...
namespace DiscordRPC.RPC
{
    ...
    internal class RpcConnection : IDisposable
    {
        ...
        private void MainLoop()
        {
            ...
            Timer = new(POLL_RATE);
            Timer.Elapsed += WaitHandleEvent;
            Timer.Start();
        }

        private void WaitHandleEvent(object sender, ElapsedEventArgs args)
        {
            // Exit if timer has already been disposed (nulled)
            if (Timer == null)
            {
                return;
            }
            ...
            try
            {
                ...
                if (aborting || shutdown)
                {
                    ...
                    if (Interlocked.Exchange(ref Timer, null) is { } oldTimer)
                    {
                        oldTimer.Stop();
...
#pragma warning restore CS0618
                ...
                if (!aborting && !shutdown && !namedPipe.IsConnected)
                {
